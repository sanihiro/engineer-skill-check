class EmployeesController < ApplicationController
  ...
  def index
    @employees = Employee.active.order("#{sort_column} #{sort_direction}").page(params[:page]).per(2)
  end
  ...
  def employee_params
    params.require(:employee).permit(:number, :last_name, :first_name, :account, :password, :e_mail, :date_of_joining, :department_id, :office_id, :employee_info_manage_auth, :news_posting_auth)
  end
